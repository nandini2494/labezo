...
<html lang="en">
    ...
    <body>
        ...
        <div class="wrapper" id="wrapper">
            <div class="left-container" id="left-container">
                ...
                <div class="left-sidebar new_left_slide_bar" id="show-nav">
                    <ul class="side-nav">
                        ...
                        <li>
                            <a href='New_Seller.html'  class='active'><span class="icon-2"></span> New Seller
                            </a>
                        </li>
                        <li>
            ...
            <div class="right-container" id="right-container">
                <div class='btn_hover_work_use'>
                    <?php 
                        if(count($result) > 0) { ?>
                    <button type="button" id="delete_id_data" class="btn pull-left custom_button">Delete <i class="fa fa-caret-down" aria-hidden="true"></i> </button>
                        <?php } ?>
                    <ul>
                        <li id="check_data"><span>Check All</span></li>
                        <li id="uncheck_data" style="display: none;"><span>Un Check All</span></li>
                    </ul>
                </div>
                <div class='clearfix'></div>
                ...
                <div class="container-fluid" style='margin-top:3em;'>
                    <div class="accordion">
                        ...
                        <dt class="dt_data">
                            <input type="checkbox" class="dt_data_checked" value="<?php echo $data['seller']['id'] ?>">
                                    ...
                                    <div>
                                        ...
                                        <div class="col-md-4 col-sm-6 col-xs-12">
                                            <div class="text-center">
                                                ...
                                                <img src="<?php echo DOMAIN_NAME ?><?php echo $data['seller']['profile_pic'] ?>" class="avatar img-circle img-thumbnail" alt="avatar">
                                                    <?php } else { ?>
                                                <img src="http://lorempixel.com/200/200/people/9/" class="avatar img-circle img-thumbnail" alt="avatar">
                                                    <?php } ?>

                                            </div>
                                            <button type="button" id="verify" class='pull-right ed_t-button_detail edit_detail' onclick='verify_seller(<?php echo $data['seller']['id'] ?>)'>Verify</button>
                                        </div>
                                        <!-- edit form column -->
                        ...
                        <?php } } else { ?>
                        <dt class="new_seller">
                        <center>No New Sellers are Available</center>
                        </dt>
                        <?php } ?>
                    </div>
    ...
    <script>
        function verify_seller(seller_id) {
            $.ajax({
                url: "<?php echo Router::url(['controller' => 'admins/update_user']) ?>",
                data: {"id": seller_id},
                type: "POST",
...
//function
    ...
    <script>
        $(document).ready(function () {
            $('#check_data').click(function () {
                $('.dt_data_checked').each(function () {
                    this.checked = true;
                    $("#uncheck_data").show();
                    $("#check_data").hide();
                });
            });
            $('#uncheck_data').click(function () {
                $('.dt_data_checked').each(function () {
                    this.checked = false;
                    $("#uncheck_data").hide();
                    $("#check_data").show();
    ...
    <script>
        $("#delete_id_data").click(function () {
            var delete_id = [];
            $('.dt_data_checked:checked').each(function (i) {
                delete_id[i] = $(this).val();
                if (delete_id.length != 0) {
                    if (confirm("Are you sure")) {
                        $.ajax({
                            url: "<?php echo Router::url(['controller' => 'admins/delete_seller']) ?>",
                            type: "POST",
                            data: {'delete_seller_id': delete_id},
                            success: function (data) {
                                location.reload();
                            }
                        });
